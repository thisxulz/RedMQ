...
<html lang="zh-CN">
  ...
  <body class="nav-md">
    ...
    <script type="text/javascript">
    		$().ready(function(){
    			$.get('/api/getAllGroups',function(result){
    				if(result.success){
    					var html = '';
    					for(var i=0;i<result.data.length;i++){
    						html += '<tr>'
    	                          +'<th scope="row">'+(i+1)+'</th>'
    	                          +'<td>'+result.data[i]+'</td>'
    	                          +'<td><button data-id="'+result.data[i]+'" class="getGroupTopics">查看该分组监听的topic</button></td>'
    	                          +'</tr>'
    					}
    			...
    			function initClickEvent(){
    				$('.getGroupTopics').on('click',function(){
    					$.get('/api/getGroupTopics/'+$(this).attr('data-id'),function(result){
        					if(result.success){
